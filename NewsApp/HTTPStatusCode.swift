...
enum HTTPStatusCode: Int, Error {
    ...
    //MARK:  - ResponseType
    enum ResponseType {
        /// - information: Этот код указывает на предворительный ответ, который содержит только строку состояния и опциональный заголовк, завершается пустой строкой
        case information
        
        /// - serverError: Этот код указывает, что серверу не удалось выполнить запрос
        case serverError
        
        /// - clientError:  Этот код используется для состояния, когда клиент может ошибиться
        case clientError
        
        /// - undefined: Код состояния не может быть определен
        case undefined

        /// - redirection: Этот код указывает, что клиент должен выполнить дополнительные действия для выполнения запроса
        case redirection
        
        /// - success: Этот код указывает, что действие, запрошенное клиентом, успешно полученно и успешно обработанно
        case success
    }
    
    // MARK: - Information 1xx
    
    /// - contin:  Сервер получил заголовок и клиент должен отправить тело запроса
    case contin = 100
    
    /// - switchingProtocol:  Сервер переключил протоколы
    case switchingProtocol = 101
    
    /// - processing: Сервер получил и обрабатывает запрос, но ответ пока не поступил
    case processing = 102
    ...
    // MARK: - Success  2xx
      ...
      case ok = 200
       ...
       /// - created: Запрос был выполнен и был создан новый ресурс
       case created = 201
       
       /// - accepted: Запрос принят в обработку, но обработка пока не завершилась
       case accepted = 202
       
       // - nonAuthoritativeInformation: При возвращении изменноной версии ответа источника , может быть из-за преобразующего прокси сервера (веб- ускоритель)
       case nonAuthoritativeInformation = 203
       
       /// - noContent:  Сервер успещно обработал запрос и не возращет никакого контента
       case noContent = 204
       
       /// - resetContent: Сервер успещно обработал запрос и не возращет никакого содержимого
       case resetContent = 205
       
       /// - partialContent:  Сервер возвращает только часть ресурса, из диапазона заголовка отправленного клиентом
       case partialContent = 206
       
       /// - multiStatus:  XML- сообщение  и может содержать несколько отдельных кодов, в зависимости от кол-ва подзапросов
